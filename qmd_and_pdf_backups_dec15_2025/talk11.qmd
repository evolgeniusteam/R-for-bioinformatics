---
title: "R for bioinformatics, Linear and nonlinear regression"
...
```{r fig.height=3, fig.width=6, message=FALSE, warning=FALSE, error=FALSE}
...
```{r fig.width=10, fig.height=4, error=FALSE, message=FALSE, warning=FALSE}
set.seed(20160227)
x<-seq(0,50,1)
...
install packages `aomisc` and `statforbiology`:

```{r eval=FALSE}
install.packages("statforbiology", "aomisc");
```

```{r using-degradation-aomisc, warning=FALSE, error=FALSE, message=FALSE}
library(aomisc); library(drc); library(statforbiology); ## updated on Oct 2025
data(degradation);
m14 <- drm(Conc ~ Time, fct = EXD.3(), data = degradation)
summary(m14)
```
...
data(speciesArea)
m15 <- drm(numSpecies ~ Area, fct = statforbiology::DRC.powerCurve(),
             data = speciesArea)
summary(m15)
```

**重点**： 1. `DRC.powerCurve` 作为建模的参数, 来自 `library(statforbiology)`
...
`aomisc`包、`statforbiology`包和 `drc`包带了非常多数据生成的函数，其使用示例如下：
...
![](images/talk11/kfold_ntimes_additional_test.png){height="70%"}
...
## Parameter tuning, plot results, cont.
...
cv_lm <- train(
  Sale_Price ~ .,  data = ames, method = "lm",
