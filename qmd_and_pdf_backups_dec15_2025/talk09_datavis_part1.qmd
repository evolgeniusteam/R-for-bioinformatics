---
title: "R for bioinformatics, data visualisation, part 1"
...
使用系统自带的 ``` swiss ``` 数据举例，它包含了47个法语地区的一些社会经济指标。
...
ggplot( swiss, aes( x = Education, y = Fertility ) ) + 
  ...
  xlab( "Education" ) + ylab( "Fertility" ) + ggtitle( "Swiss data 1888" );
```
...
```{r eval=FALSE}
## 创建一个新设备/ pdf 文件 
pdf( file = "/path/to/dir/<file_name>.pdf", height = 5, width = 5 ); 
plot(1:10); ## 作图；
dev.off(); ## 关闭设备
```
...
![default discrete colour palette](images/talk09/ggplot_default_palette.png){height=50%}
...
mpg %>% ggplot( aes(displ, hwy) ) + 
  geom_point( aes( colour = cyl, fill = class ), 
              shape = 21, size = 3, stroke = 2); ## 注意 shape 等参数在 aes() 内部与外部的区别
```
...
用特定的**数值比例** (ratio，即：**y 和 x 的真实值**) 作图，如下图所示：
