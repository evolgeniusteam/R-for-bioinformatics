---
title: "R for bioinformatics, R language basics, part 3: factor"
...
![two R sessions](images/talk04/r_sessions.png){height="60%"}

## R session in RStudio

Each RStudio session is automatically associated with a R session

![R session in RStudio](images/talk04/r_session_in_rstudio.png){heigh="30%"}

## start a new RStudio session by creating a new project

1.  右上角的 Project 按钮，在弹出菜单里选 New Project …

![create new project, step 1](images/talk04/rstudio_create_new_project.png){height="60%"}

## create a new project, cont.

2.  Select: New directory -\> New Project in the popup window

![create new project, step 2](images/talk04/rstudio_create_project_2.png){height="60%"}

## create a new project, cont.

3.  Enter a new directory name, choose its mother directory ...

![create new project, step 3](images/talk04/rstudio_create_new_project_3.png){height="60%"}
...
![RStudio enviroment window](images/talk04/rstudio_enviroment_window.png){height="70%"}
...
![Project options](images/talk04/rstudio_project_options.png){height="50%"}
...
![Open a project](images/talk04/rstudio_open_a_project.png){height="60%"}
...
**注意**：用 `as.factor` 创建 factor 时，得到的 levels 按`字母表`排列；
...
**注意**：用 `levels( y )` 方式指定 levels 时，则按照指定的顺序；
...
( responses <- factor( c("Agree", "Agree", "Strongly Agree", "Disagree", "Disagree", "Agree"), 
                       levels = c( "Disagree", "Agree", "Strongly Agree"), ordered = T ) );
...
先读取一个文件，注意 `stringsAsFactors` 参数（默认为 `F`）；
...
**上面作图的问题！！**：`subset()` 无法去除不用的 factors ...

解决方案：使用 `droplevels` 函数（see next page）。
...
## 也可以使用 tibble ，默认不使用 `factor`, cont.
