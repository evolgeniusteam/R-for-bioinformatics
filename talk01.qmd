---
title: "R for bioinformatics, Multi-omics data analysis and visualisation, #1"
...
![Briefings in Bioinformatics (2023 IF=9.5)](images/talk01/ningkang_2023_bib.png){width="80%"}

基于肿瘤菌的多种肿瘤诊断模型

## 薛宇

![Nature BME & Immunity (2023 IF=29)](images/talk01/ML-xueyu.png){width="60%"}

机器学习在新冠标志物和图像识别当中的应用

## 薛宇, cont.

![Nature communications (2023 IF=16.6)](images/talk01/xueyu2023_NC.png){width="60%"}

小样本机器学习

## 陈卫华

![Cell Metabolism (2023IF:29)](images/talk01/ML-weihuachen.png){width="60%"}

机器学习在肠道菌群为基础的疾病标志物挖掘当中的应用

## 基于肠道菌群的疾病诊断

![Gut Microbes (2023 IF: 12.2)](images/talk01/WHC_2023_gm.png){height="75%"}

基于肠道菌群的机器学习诊断模型

## 基于肠道菌群的预后分析

![Gut (2023 IF: 24)](images/talk01/WHC_2022_gut.png){width=95%}

肠道菌群可预示NKTCL的预后

## 其它动物的肠道菌群

![Nature communications (2023 IF: 16.6)](images/talk01/buffalo_MAGs.png){width=100%}

## 噬菌体研究

![Advanced science (2023 IF: 15.1)](images/talk01/WHC_2023_advanced_science.png){width=90%}
...
1. 开卷；允许带资料、书籍，不允许带任何有大屏幕或智能屏幕的电子设备
...
cited 242 times in 2024 (ver2+3), 170 so far (as of Aug 30, 2025)
...
大部分包都由 Posit （前身为RStudio）公司提供；包括：ggplot2，tidyr，readr，stringr等。可以用以下命令一次性安装。不过，为方便读者直接从后面章节阅读，在每一次使用新包时，我们会再次进行提示安装方法。
...
<https://shiny.posit.co/r/gallery/interactive-visualizations/bus-dashboard/>](images/talk01/shiny_example.png){height="40%"}

## RStudio packages for data visualisation, cont.

### other packages（默认已安装）
...
### 2，安装 `tinytex` 包，辅助将 Rmd 文件转为 PDF
...
### 6，打开 `R for bioinformatics.Rproj` 项目
...
### 7，通过 Rstudio 打开相应的 `Excercises and homework` 文件夹下面的 `talk01-homework.Rmd` 文件
