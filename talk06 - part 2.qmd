...
![Example final outcomes](images/talk06/igraph_example.png){height="50%"}
...
![The tara oceans expedition](images/talk06/tara_oceans_boat.jpg){height="60%"}
...
![human gut microbita](images/talk06/GutMicrobeInfographic.png){height="70%"}
...
![ 整合后的结果示意 ](images/talk06/table_data_integration.jpg){height="50%"}

## 第一种方法，使用 pivot_wider

思路：先合并，再 pivot_wider 
...
expr2 <- expr %>% mutate( group = "rkpm" ) %>%
  select( gene, group, value=rkpm ) %>% group_by( gene ) %>% slice( 1 );
```

## 第一种方法，使用 pivot_wider，准备数据, cont.
